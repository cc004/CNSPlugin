...
namespace PrismaticChrome.Shop
{
    public abstract class StorageProvider<T> : IStorageProvider
    {
        ...
        protected abstract bool TryTakeFrom(TSPlayer player, int count, out T content);
        ...
        public bool TryTakeFrom(TSPlayer player, int count, out string content)
        {
            var result = TryTakeFrom(player, count, out T t);
